...
For user API keys in Fentaris CLI 1.1.0 and newer, use `fentaris auth api-key` instead of custom scripts or direct encryption helpers. Prefer `fentaris auth api-key add <user-id> --value-stdin` for provided keys, `fentaris auth api-key add <user-id> --generate` for generated keys, `fentaris auth api-key list --user <user-id> --json` for inspection, and `fentaris auth api-key remove <user-id> --value-stdin` for removal. Clients send keys in the `x-fentaris-api-key` header.
